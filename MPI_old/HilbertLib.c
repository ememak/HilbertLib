...
HilbertLibCurveSortComparator (const void *_elem1, const void *_elem2)
{
	...
	return (HilbertPos[pos] < HilbertPos[pos2] ? -1 : HilbertPos[pos]>HilbertPos[pos2]);
}
	...
	while (bsright / bsleft > 1 + HILPOS_EPS)
	  {			// CHANGE FOR UNSIGNED INTS
		  ...
		  if (suma > particlesRate)
		    {
			    bsright = bsmiddle - HILPOS_MARGIN;
		    }
		  else
		    {
			    bsleft = bsmiddle;
	...
	)
{
	...
	hilpos_t *binsBoundaries = calloc (NodesCount,
					   sizeof (hilpos_t));
	hilpos_t lastUsed = MinCoord-HILPOS_MARGIN;
			    ...
			    int RootRank, hilpos_t * HCoordinates)
{
	...
	MPI_Gather (&sendBuff, 1, MPI_INT, NULL,
		    0, MPI_INT, RootRank, MPI_COMM_WORLD);
		    ...
		    MDPoint * *NewData, int *NewDataCount)
{
	...
	coord_t *sendBuf = calloc (MyPointsCount * Dimensions,
				   sizeof (coord_t));
	assert ((MyPointsCount==0)^(sendBuf != NULL));
	assert ((MyPointsCount==0)^(tagSendBuf != NULL));
		     ...
		     MDPoint * *NewDataPtr, int *NewDataSize)
{
	...
	if (rank == RootRank)
	...
	else
	  {
		  ...
		  boundaries =
			  HilbertLibRecvBoundariesFromRoot(size, RootRank);
	  }
	/*if (rank == 0)
		for (i = 0; i < size; i++)
		  {
...
void
sendQuery (coord_t * LD, coord_t * RD, int NumberOfProcesses, int Dimensions, int MyRank, int *Counter,	// unique (for query) number in <0,n-1>, where n is # of querie
	   ...
	   int numberOfMyQueries)
{
	...
	if (*BigBuff == NULL)
	  {
		*BigBuff = malloc (number_of_bytes
				  * numberOfMyQueries);
		memset(*BigBuff, 0, number_of_bytes * numberOfMyQueries);
	  }
	unsigned char *buff =
		(*BigBuff) +
	...
	memcpy (buff +
		...
		Counter, sizeof (int));
	/*MPI_Ibcast (buff, number_of_bytes, //connects with ibcast in answerQueries
		    MPI_BYTE, MyRank, //Nice idea, but don't work...
		...
		MPI_Request * Request)
{
	...
	for (i = 0; i < DataSize; i++)
		boolSend[i] = -1;
	unsigned char **realBuffs = calloc (NumberOfProcesses, sizeof (unsigned char *));
	...
	for (i = 0; i < NumberOfProcesses; i++)
	  {
		  ...
		  for (j = 0; j < RecvCount[i]; j++)
		    {
			    ...
			    if( i == MyRank ) {
					...
					queryRank[j] = *(int*)(BigBuff + j*number_of_bytes + sizeof(coord_t)*Dimensions*2);//ew + sizeof(int)
					printf("QueryRank[%d]=%d\n", j, queryRank[j]);
					fflush(stdout);
					MPI_Bcast((BigBuff + j*number_of_bytes), number_of_bytes, MPI_BYTE, i, MPI_COMM_WORLD);
					//MPI_Wait(Request, MPI_STATUS_IGNORE);
			    } 
				else {
				    //MPI_Request req1;
				    /*MPI_Ibcast (buff, number_of_bytes, //Ibcast in sendQuery sends
						MPI_BYTE,		//Nice Idea, but don't work...
						i, MPI_COMM_WORLD, &req1);*/
				    MPI_Bcast(buff, number_of_bytes, MPI_BYTE, i, MPI_COMM_WORLD);
				    //MPI_Wait (&req1, MPI_STATUS_IGNORE);
				    LD = (coord_t *) buff;
				    RD = (coord_t*) (buff + Dimensions* sizeof (coord_t));
				    queryRank[j] = *((int*) (buff + sizeof(coord_t)* Dimensions * 2));
			    }
			    /*printf("rank : %d Dostalem zapytanie %d od procesu %d:\n",MyRank,queryRank[j], i);
			    printf("LD : ");
		  ...
		  for (j = 0; j < allResults; j++)
		    {
			whoseIsThatPoint[j]=toSort[j].b;
		    }
		  int *info_buff = calloc (2, sizeof (int));
		  info_buff[0] = allResults;
		  info_buff[1] = resultsCount;
		  printf("Wysylam %d->%d, info_buff[0]: %d, info_buff[1]: %d\n", MyRank, i, info_buff[0], info_buff[1]);
		  fflush(stdout);
		  MPI_Request req_null;
		  MPI_Isend (info_buff, 2,
			     MPI_INT, i, ANSWER_TAG-1, MPI_COMM_WORLD, &req_null);
		...
		if(MyRank==15 && i==8){
		printf("Wysylam bigbuff %d->%d\n", MyRank, i);
		  for(j=0; j<real_buff_size; j++){
		  printf("%d ", real_buff[j]);
		  }
		  printf("Koncze wysylac bigbuff %d->%d\n", MyRank, i);
		  fflush(stdout);
		  }
		  ...
		  MPI_Isend (real_buff,
			     real_buff_size,
			     MPI_BYTE, i, ANSWER_TAG+i, MPI_COMM_WORLD, &Request[i]);
...
void
recvQueries (			// In comments - old code, old comment here - "Add MPI_TEST_SOME to fasten" (in some meaning done)
		    ...
		    int MyRank)
{
	...
	//MPI_Request req1;
	MPI_Request* req=calloc(ProcessCount, sizeof(MPI_Request));
	for (i = 0; i < ProcessCount; i++)
	  {
		  if (i == MyRank)
		    {
		  ...
		  MPI_Irecv (cntBuf, 2, MPI_INT,
			     i, 765, MPI_COMM_WORLD, &req1);
		    MPI_Wait(&req1, MPI_STATUS_IGNORE);
		  (*NewNeighboursSize) += cntBuf[1];*/
		  MPI_Irecv(cntBuffers[i], 2, MPI_INT, i, ANSWER_TAG-1, MPI_COMM_WORLD, &req[i]);
	  }
	for(i=0; i<ProcessCount-1; i++){
		MPI_Waitany(ProcessCount, req, &j, MPI_STATUS_IGNORE);
		(*NewNeighboursSize) += cntBuffers[j][1];
	}
	...
	for (i = 0; i < ProcessCount; i++)
	  {
		  ...
		  printf("odbieram %d->%d, cntCuf[0]: %d, cntBuf[1]: %d\n", i, MyRank, cntBuf[0], cntBuf[1]);
		  fflush(stdout);
		  unsigned char *big_buff = malloc (big_buff_size);
		  memset(big_buff, 0, big_buff_size);
		  /*MPI_Irecv (big_buff,
			     big_buff_size,
			     MPI_BYTE, i, 766, MPI_COMM_WORLD, &req1); // irecv+wait=recv
		  MPI_Wait (&req1, MPI_STATUS_IGNORE);*/
		  MPI_Recv(big_buff, big_buff_size, MPI_BYTE, i, ANSWER_TAG+MyRank, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
		  unsigned char *big_buff_ptr = big_buff;
		  if(MyRank==8 && i==15){
		  printf("Odbieram bigbuff %d->%d\n", i, MyRank);
		  for(j=cntBuf[1]*MDPOINT_FULL_SIZE(Dimensions); j<big_buff_size; j++)
		  printf("%d ", big_buff[j]);
		  printf("\n");
		  printf("Koncze odbierac bigbuff %d->%d", i, MyRank);
		  fflush(stdout);
		  }
		  for (j = 0; j < cntBuf[1]; j++)
		    {
		  ...
		  for (j = 0; j < cntBuf[0]; j++)
			...
			pointPtr += 1;
			if(actVal!=0){
			printf("Bad actVal[%d]=%d, Rank: %d\n", j, actVal, MyRank);
			break;
			}
			PtrVectorPB(&VecResults[actVal],
				&((*NewNeighbours)
				[lastNewNeighboursPtr + pointPtr]));
	...
	for (i = 0; i < selfQueriesCount; i++)
	  {
		...
		{
		PtrVectorPB(&VecResults[SelfQueriesRank[i]], 
				SelfQueriesResult[i][j]);
		}
	  }
	free (cntBuffers);
	(*ResultsSize) = calloc(QueriesCount, sizeof(int));
	(*Results) = calloc(QueriesCount, sizeof(MDPoint**));
	for (i = 0; i < QueriesCount; i++)
	  {
		  (*Results)[i] =
		  ...
		  memcpy ((*Results)[i],
			...
			(*ResultsSize)[i]=VecResults[i].size;
